// credit: https://github.com/rtmacaibay/memory-allocator
/**
 * allocator.c
 *
...
// m: 
//  * c string marco concat - https://stackoverflow.com/questions/5256313/c-c-macro-string-concatenation
#define LOG(fmt, ...) \
...
void *reuse_space(struct mem_block * ptr, size_t real_sz) {
    ...
    if (ptr->usage == 0) {
        ...
        // |---- mem_block meta info -- | ---- available space --- |
        // ^ ptr
        //                              ^ ptr+1
        return ptr + 1; 
    }
    ...
    //
    // the pointer itself is the start address
    ...
    if (g_tail == ptr) {
        // head-> block 1 -> block 2 -> g_tail
        //                              ^ ptr
        // head-> block 1 -> block 2 -> ptr -> freeloader-
        //                                     ^ g_tail
...
void free(void *ptr) {
    ...
    //get the struct
    // the meta infomation is stored at each mem_block
